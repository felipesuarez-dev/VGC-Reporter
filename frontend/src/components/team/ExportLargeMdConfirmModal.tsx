...
export function ExportLargeMdConfirmModal({ open, count, onConfirm, onCancel }: Props) {
    ...
    >
      ...
      >
        <button
          ...
          onClick={onCancel}
          aria-label={t("top_teams.export_warning_cancel")}
        >
          <X size={16} />
        </button>
        ...
        <div className="mb-3 flex items-start gap-2">
          ...
          <div>
            <h2 className="text-base font-semibold" style={{ color: "var(--text)" }}>
              {t("top_teams.export_warning_title")}
            </h2>
            <p className="mt-1 text-xs" style={{ color: "var(--text-muted)" }}>
              {t("top_teams.export_warning_body", { count })}
            </p>
          </div>
        </div>
        ...
        <label className="mt-3 flex items-center gap-2 text-xs" style={{ color: "var(--text)" }}>
          ...
          />
          {t("top_teams.export_warning_dont_ask")}
        </label>

        <div className="mt-4 flex justify-end gap-2">
          <button type="button" className="btn-ghost" onClick={onCancel}>
            {t("top_teams.export_warning_cancel")}
          </button>
          <button type="button" className="btn-primary" onClick={() => onConfirm(dontAsk)}>
            {t("top_teams.export_warning_confirm")}
          </button>
        </div>
      </div>
    </div>
